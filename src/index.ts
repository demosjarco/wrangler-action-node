...
class Wrangler {
	...
	private main_command(INPUT_COMMAND: string, INPUT_ENVIRONMENT: string, INPUT_VARS: string[]): Promise<void> {
		...
		if (INPUT_COMMAND.length === 0) {
			...
			if (INPUT_ENVIRONMENT.length === 0) {
				return new Promise((resolve, reject) => {
					exec(`npx ${wranglerCommand} ${deployCommand} ${envVarArgument}`.trim(), { cwd: this.workingDirectory, env: process.env }, (error, stdout, stderr) => {
						if (error) {
							console.error(error);
							core.setFailed(error.message);
							reject(error);
				...
				return new Promise((resolve, reject) => {
					exec(`npx ${wranglerCommand} ${deployCommand} --env ${INPUT_ENVIRONMENT} ${envVarArgument}`.trim(), { cwd: this.workingDirectory, env: process.env }, (error, stdout, stderr) => {
						if (error) {
							console.error(error);
							core.setFailed(error.message);
							reject(error);
