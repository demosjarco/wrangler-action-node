...
/* harmony import */ var node_path__WEBPACK_IMPORTED_MODULE_1__ = __nccwpck_require__(411);
...
/* harmony import */ var node_child_process__WEBPACK_IMPORTED_MODULE_2__ = __nccwpck_require__(718);
...
class Wrangler {
    API_CREDENTIALS = '';
    workingDirectory = this.setupWorkingDirectory(_actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput('workingDirectory'));
    WRANGLER_VERSION = 3;
    CF_API_TOKEN;
    ...
    async main() {
        await this.installWrangler(_actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput('wranglerVersion'));
    ...
    setupWorkingDirectory(workingDirectory = '') {
        let normalizedPath = '';
        try {
            normalizedPath = node_path__WEBPACK_IMPORTED_MODULE_1___default().normalize(workingDirectory);
        }
        catch (error) {
            console.error(workingDirectory, 'not a valid path', error);
            console.warn('Ignoring `workingDirectory` and using current directory');
            normalizedPath = node_path__WEBPACK_IMPORTED_MODULE_1___default().normalize('');
        }
        return normalizedPath;
    }
    installWrangler(INPUT_WRANGLERVERSION) {
        ...
        return new Promise((resolve, reject) => {
            (0,node_child_process__WEBPACK_IMPORTED_MODULE_2__.exec)(command, { cwd: this.workingDirectory, env: process.env }, (error, stdout, stderr) => {
                if (error) {
                    console.error(error);
    ...
    authenticationSetup(INPUT_APITOKEN, INPUT_APIKEY, INPUT_EMAIL, INPUT_ACCOUNTID) {
        ...
        if (INPUT_APIKEY.length === 0 && INPUT_EMAIL.length !== 0) {
            _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed("Provided an email without an API key for authentication. Please pass in 'apiKey' and 'email' to the action.");
        }
        if (this.API_CREDENTIALS.length === 0) {
            _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed("Unable to find authentication details. Please pass in an 'apiToken' as an input to the action, or a legacy 'apiKey' and 'email'.");
        }
        else {
    ...
    execute_commands(commands) {
        return new Promise(async (mainResolve, mainReject) => {
            ...
            if (childError) {
                _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed('command failure');
                mainReject();
            }
            else {
                mainResolve();
    ...
    putSecrets(INPUT_SECRETS, INPUT_ENVIRONMENT) {
        return new Promise(async (mainResolve, mainReject) => {
            let childError = false;
            for (const secret of INPUT_SECRETS) {
                ...
                await new Promise((childResolve, childReject) => {
                    const child = (0,node_child_process__WEBPACK_IMPORTED_MODULE_2__.spawn)(secretCommand.shift(), secretCommand, { cwd: this.workingDirectory, env: process.env, stdio: 'pipe' });
    ...
    main_command(INPUT_COMMAND, INPUT_ENVIRONMENT, INPUT_VARS) {
        ...
        if (INPUT_COMMAND.length === 0) {
            ...
            if (INPUT_ENVIRONMENT.length === 0) {
                return new Promise((resolve, reject) => {
                    (0,node_child_process__WEBPACK_IMPORTED_MODULE_2__.exec)(`npx ${wranglerCommand} ${deployCommand} ${envVarArgument}`.trim(), { cwd: this.workingDirectory, env: process.env }, (error, stdout, stderr) => {
                        if (error) {
                            console.error(error);
                            _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed(error.message);
                            reject(error);
                ...
                return new Promise((resolve, reject) => {
                    (0,node_child_process__WEBPACK_IMPORTED_MODULE_2__.exec)(`npx ${wranglerCommand} ${deployCommand} --env ${INPUT_ENVIRONMENT} ${envVarArgument}`.trim(), { cwd: this.workingDirectory, env: process.env }, (error, stdout, stderr) => {
                        if (error) {
                            console.error(error);
                            _actions_core__WEBPACK_IMPORTED_MODULE_0__.setFailed(error.message);
                            reject(error);
